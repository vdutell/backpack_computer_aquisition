...
def apply_cam_settings(cam, timing_mode=None, exposure=None, framerate=None, gain=None):
    """
...
def aquire_save_filewise(num_frames=10, cam_timing_mode='XI_ACQ_TIMING_MODE_FREE_RUN', cam_exposure=1000, 
                         cam_framerate=500, cam_gain=1,
                         verbose=True):
    """
    ...
    try:
        ...
        cam = apply_cam_settings(cam, timing_mode=cam_timing_mode, exposure=cam_exposure, 
                                framerate=cam_framerate, gain=cam_gain)
        if(verbose):
        ...
        if(verbose):
            print('Starting data acquisition...',end='')
